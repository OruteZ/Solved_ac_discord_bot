...
async def 유저등록(ctx, message, Discord_id = None):
    ...
    if(Discord_id == None) : Discord_id = ctx.author
    else :
        mentioned_members = ctx.message.mentions
        if(mentioned_members == []) : Discord_id = ctx.author
        else : Discord_id = mentioned_members[0]

    if(users.get(Discord_id.id) != None) :
    ...
    embed = discord.Embed(color=tier_color, title = f"PROFILE : {info['id']}", url = f"https://solved.ac/profile/{info['id']}")
...
async def 랜덤백준(ctx, tier = '', tags = ''):
    ...
    problem = BOJ_random_defense(Tier = tier, Tag = tags)
    if(problem == None) :
